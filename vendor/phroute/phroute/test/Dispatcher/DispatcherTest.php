...
class DispatcherTest extends \PHPUnit\Framework\TestCase {
    ...
    public function testReverseRouteWithDashes()
    {
        ...
        $r->any( array('customer-catalogue/store/{store:i}?', 'products'), array(__NAMESPACE__.'\\Test','route'));

        $this->assertEquals('customer-catalogue/store', $r->route('products'));
        $this->assertEquals('customer-catalogue/store/1', $r->route('products', array(1)));
    }
    ...
    public function testPrefixGroupsReverseRoutes()
    {
        ...
        $r->group(['prefix' => 'customer-catalogue/store'], function($r) {
            $r->any(['items/{store:i}?', 'products'], array(__NAMESPACE__.'\\Test','route'));
        });

        $this->assertEquals('customer-catalogue/store/items', $r->route('products'));
        $this->assertEquals('customer-catalogue/store/items/1', $r->route('products', array(1)));
    }
